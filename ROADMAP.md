...
Select one public perturbation dataset, favoring an immune-cell setting where feasible. The current implementation uses GSE90063 (Dixit et al., 2016), a mouse BMDC Perturb-seq study with unstimulated and LPS 3-hour contexts. Cellxgene may be used for targeted discovery and AnnData retrieval if the study is available there; the current source path uses GEO supplementary files. Document provenance, release or snapshot, access conditions, assay semantics, and limitations. Load each condition as cells-by-genes AnnData; validate dimensions, metadata, perturbation assignments, controls, and gene identifiers. Inspect context and assignment coverage, canonicalize relevant labels, and identify missingness and confounding. Keep deterministic preprocessing separate from source validation.

**Completion:** the released GSE90063 files can be ingested repeatably; validation checks detect malformed identifiers, missing files, invalid perturbation/control assignments, and matrix/index alignment problems. The current report supports unseen-perturbation and LPS-context evaluation, while donor and cell-type OOD dimensions are explicitly unsupported.
